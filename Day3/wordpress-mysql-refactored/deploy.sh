...
oc apply -f mysql-setup-cm.yml
oc apply -f mysql-statefulset.yml

echo "\nWaiting for all MySQL pods to be Ready..."
...
echo "  Sidecar logs:       oc logs mysql-0 -c setup"
echo "  Replication status: oc exec mysql-1 -c mysql -- mariadb -uroot -proot@123 -e 'SHOW SLAVE STATUS\G'"
